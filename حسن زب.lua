...
local Settings = {
    ...
    Aimbot = {
        ...
        MaxDistance = 200,
        ShowFOV = false,
        TargetPart = "Head", -- Head, Torso, Leg
        SmoothAim = 50,
        MagicAim = false,
        AutoFire = false,
        FullHead = false
...
-- دالة العثور على أفضل هدف مع تحديد الجزء المستهدف
local function GetTargetPart(character, partType)
    ...
    elseif partType == "Leg" then
        ...
        return character:FindFirstChild("Right Leg") or character:FindFirstChild("RightLowerLeg")
    end
    return character:FindFirstChild("Head")
end
...
local function FindBestTarget()
    ...
    for _, Player in ipairs(Players:GetPlayers()) do
        if Player ~= LocalPlayer and Player.Character then
            ...
            local TargetPart = GetTargetPart(Player.Character, Settings.Aimbot.TargetPart)
            if TargetPart then
...
local function TeleportBehindTarget(targetPlayer)
    ...
    if not targetRoot or not localRoot or not targetHumanoid or targetHumanoid.Health <= 0 then return end
    
    -- حساب الموقع خلف الهدف بمسافة 3 متر
    local targetCF = targetRoot.CFrame
    local behindPosition = targetCF.Position - targetCF.LookVector * 3
    
    -- التحقق من وجود جدار (اختياري)
    local rayParams = RaycastParams.new()
    rayParams.FilterDescendantsInstances = {localChar, targetPlayer.Character}
    rayParams.FilterType = Enum.RaycastFilterType.Blacklist
    
    local rayResult = workspace:Raycast(targetCF.Position, (behindPosition - targetCF.Position).Unit * 3, rayParams)
    if rayResult then
        -- إذا وجد جدار، حاول تعديل الموقع قليلاً
        behindPosition = rayResult.Position + rayResult.Normal * 2
    end
    
    -- تطبيق التيلبورت
    localRoot.CFrame = CFrame.new(behindPosition) * CFrame.Angles(0, targetCF.Rotation.Y, 0)
end

-- إنشاء زر Teleport القابل للسحب
local function CreateTeleportButton()
    ...
    TeleportButton.MouseEnter:Connect(function()
        TweenService:Create(TeleportButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.new(0.3, 0.3, 0.3)}):Play()
    end)
    
    TeleportButton.MouseLeave:Connect(function()
        TweenService:Create(TeleportButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.new(0.2, 0.2, 0.2)}):Play()
    end)
    ...
    TeleportButton.MouseButton1Click:Connect(function()
        if CurrentTarget then
            ...
            -- إذا لم يوجد هدف، ابحث عن أقرب هدف
            local bestTarget, _ = FindBestTarget()
            if bestTarget then
    ...
    TeleportButton.Visible = Settings.Combo.TeleportPlayer.Enabled
end
...
MainFrame.Size = UDim2.new(0, 370, 0, 500) -- زيادة الارتفاع
...
do
    local y = 10
    y = CreateToggle(ESPContent, "ESP", function() return Settings.ESP.Enabled end, function(v) Settings.ESP.Enabled = v end, y)
    y = CreateToggle(ESPContent, "Snapline", function() return Settings.ESP.SnaplineEnabled end, function(v) Settings.ESP.SnaplineEnabled = v end, y)
    y = CreateDropdown(ESPContent, "Position", function() return Settings.ESP.SnaplinePosition end, function(v) Settings.ESP.SnaplinePosition = v end, {"Center", "Top", "Bottom"}, y)
    y = CreateToggle(ESPContent, "Rainbow", function() return Settings.ESP.RainbowEnabled end, function(v) Settings.ESP.RainbowEnabled = v end, y)
    y = CreateToggle(ESPContent, "Name ESP", function() return Settings.ESP.NameESP end, function(v) Settings.ESP.NameESP = v end, y)
    y = CreateToggle(ESPContent, "Team Check", function() return Settings.ESP.TeamCheck end, function(v) Settings.ESP.TeamCheck = v end, y)
    y = CreateToggle(ESPContent, "Off-Screen Arrow", function() return Settings.ESP.OffScreenArrow end, function(v) Settings.ESP.OffScreenArrow = v end, y)
    ESPContent.CanvasSize = UDim2.new(0,0,0,y)
end

-- تعبئة تبويبة Aimbot مع خيارات الرأس والجسم والقدم
do
    local y = 10
    y = CreateToggle(AimbotContent, "Aimbot", function() return Settings.Aimbot.Enabled end, function(v) Settings.Aimbot.Enabled = v end, y)
    y = CreateToggle(AimbotContent, "FOV Circle", function() return Settings.Aimbot.ShowFOV end, function(v) Settings.Aimbot.ShowFOV = v FOVCircle.Visible = v end, y)
    y = CreateInput(AimbotContent, "FOV:", function() return Settings.Aimbot.FOV end, function(v) Settings.Aimbot.FOV = v end, y, 1, 360)
    y = CreateInput(AimbotContent, "Max Distance:", function() return Settings.Aimbot.MaxDistance end, function(v) Settings.Aimbot.MaxDistance = v end, y, 1, 10000)
    y = CreateInput(AimbotContent, "Smooth Aim (1-100):", function() return Settings.Aimbot.SmoothAim end, function(v) Settings.Aimbot.SmoothAim = v end, y, 1, 100)
    y = CreateDropdown(AimbotContent, "Target Part", function() return Settings.Aimbot.TargetPart end, function(v) Settings.Aimbot.TargetPart = v end, {"Head", "Torso", "Leg"}, y)
    y = CreateToggle(AimbotContent, "Magic Aim", function() return Settings.Aimbot.MagicAim end, function(v) Settings.Aimbot.MagicAim = v end, y)
    y = CreateToggle(AimbotContent, "Auto Fire", function() return Settings.Aimbot.AutoFire end, function(v) Settings.Aimbot.AutoFire = v end, y)
    y = CreateToggle(AimbotContent, "Full Head", function() return Settings.Aimbot.FullHead end, function(v) Settings.Aimbot.FullHead = v end, y)
    AimbotContent.CanvasSize = UDim2.new(0,0,0,y)
end

-- تعبئة تبويبة Combo مع Teleport Player و 10 سرعات
do
    local y = 10
    y = CreateToggle(ComboContent, "Infinite Jump", function() return Settings.Combo.InfiniteJump.Enabled end, function(v)
    ...
    y = CreateToggle(ComboContent, "Speed", function() return Settings.Combo.Speed.Enabled end, function(v)
    ...
    y = CreateInput(ComboContent, "Speed Multiplier (1-10):", function() return Settings.Combo.Speed.Multiplier end, function(v) 
        Settings.Combo.Speed.Multiplier = v 
    end, y, 1, 10) -- زيادة المدى إلى 10
    
    y = CreateToggle(ComboContent, "No Wall", function() return Settings.Combo.NoWall end, function(v) Settings.Combo.NoWall = v end, y)
    
    y = CreateToggle(ComboContent, "Teleport Player", function() return Settings.Combo.TeleportPlayer.Enabled end, function(v)
        Settings.Combo.TeleportPlayer.Enabled = v
        if v then
...
local function ApplyHover(btn)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.2), {
            Size = origSize + UDim2.new(0,5,0,5), 
...
MinimizeButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(MainFrame, TweenInfo.new(0.3), {
        Size = Minimized and UDim2.new(0,370,0,30) or UDim2.new(0,370,0,500)
    }):Play()
...
RunService.RenderStepped:Connect(function()
    ...
    -- تحديث سرعة المشي (إصلاح مشكلة السرعة)
    if LocalPlayer.Character then
        local Humanoid = LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
        if Humanoid then
            if Settings.Combo.Speed.Enabled then
                -- حفظ السرعة الأصلية إذا لم تكن محفوظة
                if Settings.Combo.Speed.OriginalSpeed == 16 then
                    Settings.Combo.Speed.OriginalSpeed = Humanoid.WalkSpeed
                end
                -- تطبيق السرعة الجديدة
                Humanoid.WalkSpeed = 16 * Settings.Combo.Speed.Multiplier
            else
                -- استعادة السرعة الأصلية
            ...
            if BestTarget then
                if CurrentTarget and CurrentTarget ~= BestTarget then
                    if CurrentTarget.Character then
                        local TargetPart = GetTargetPart(CurrentTarget.Character, Settings.Aimbot.TargetPart)
                        if TargetPart then
...
Players.PlayerRemoving:Connect(function(plr)
    if ESP_Drawings[plr] then
        for _, d in pairs(ESP_Drawings[plr]) do
            d:Remove()
        end
...
print("Script loaded successfully with fixes and new features!")
